...
document.querySelector('[data-slider]')?.addEventListener('input',event=>{const value=Number(event.target.value);const before=document.querySelector('[data-before]');const line=document.querySelector('[data-line]');if(before)before.style.clipPath=`inset(0 ${100-value}% 0 0)`;if(line)line.style.left=`${value}%`;});
